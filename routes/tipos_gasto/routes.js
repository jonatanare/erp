...
router.route('/empresa/:empresa').get(verifyToken, isAdmin, async (req, res) => {
    try{
        ...
        let result =  await TiposGasto.findAll({
            where: {
                empresa: empresa
            },
        }
...
router.route('/:id').get(verifyToken, isAdmin, async (req, res) => {
    try{
        ...
        let result = await TiposGasto.findOne({
            where: {id: id}
        })
        ...
        if(result){
            ...
            res.status(404).send('Tipo de gasto no encontrado.')
        }
    }catch(err){
        res.status(500).send(err)
    }
...
router.route('/:id').delete(verifyToken, isAdmin, async (req, res) => {
    try{
        ...
        if(result){
            db.sequelize.query("delete from tipos_gasto where id = $1", { bind: [id] })
            res.status(200).send({'message': 'Tipo de gasto borrado'})
        }else{
            res.status(404).send('Tipo de gasto no encontrado.')
        }
...
router.route('/:id').put(verifyToken, isAdmin, async (req, res) => {
    try{
        ...
        if(result){
            await result.update(getUpdateObject(TiposGasto, data, ["id"]))
            res.send(result)
        }else{
            res.status(404).send('Tipo de gasto no encontrado.')
        }
...
router.route('/empresa/:empresa').post(verifyToken, isAdmin, async (req, res) => {
    try{
        ...
        if(!result){
            const nuevo = await TiposGasto.create(data)
            res.send(nuevo)
        }else{
            res.status(409).send('Tipo de gasto existente.')
        }
